...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    - подозрительно повторяющиеся идентификаторы;
    - слишком много уникальных значений в категориальных признаках
    и т.п.
    """
    flags: Dict[str, Any] = {}
    ...
    #------------------------------------------------------------------------------------
    flags["too_many_unique_values"] = any([s.unique > 20 for s in summary.columns if s.dtype == 'object'])
    flags["max_zeros_share"] = max([s.zeros_share for s in summary.columns])
    flags["too_many_zeros"] = flags["max_zeros_share"] > 0.6
    flags["has_suspicious_id_duplicates"] = any([summary.n_rows > s.unique for s in summary.columns if 'id' in s.name.lower()])
    #------------------------------------------------------------------------------------
    ...
    if summary.n_cols > 100:
        score -= 0.1
    #------------------------------------------------------------------------------------
    if flags["too_many_unique_values"]:
        score -= 0.05
