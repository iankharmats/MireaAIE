...
| 2  | Endpoint `/predict` использует **реальную модель**, а не заглушку        |         ✅         |  `src/api.py`, `src/caricature_generator.py`             |
| 3  | Есть EDA и хотя бы один эксперимент с метриками                          |         ✅         | `notebooks/feature_extraction`, `notebooks/dpo_train`, `report.md`     |
| 4  | Есть baseline и улучшенная модель, есть **сравнение по метрикам**        |         ✅         | `notebooks/dpo_train.ipynb` |
| 5  | Код не свален в один ноутбук: есть внятная структура в `src/`            |        ✅          | `src/api.py`, `src/feature_extraction.py`,  `src/metrics.py` и тд|
| 6  | Есть Dockerfile **или** понятный сценарий развёртывания без Docker       |       ✅           | Есть сценарий запуска FastAPI + Uvicon, раздел «Как запустить»         |
| 7  | Есть `.env.example` и **нет** в репозитории реальных секретов/паролей    |          ✅        | `configs/`|
| 8  | Реализованы логи/наблюдаемость (хотя бы консольные логи + `/health`)     |        ✅          | Использование `logging`, endpoint `/health`, `/metrics` |
