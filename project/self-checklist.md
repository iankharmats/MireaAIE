...
| 2  | Endpoint `/predict` использует **реальную модель**, а не заглушку        |         ✅         |  `src/api`, `src/caricature_generator`             |
...
| 6  | Есть Dockerfile **или** понятный сценарий развёртывания без Docker       |       -           | Используется FastAPI       |
| 7  | Есть `.env.example` и **нет** в репозитории реальных секретов/паролей    |          -        | Конфиги и секреты не используются    |
